...
import re, configparser, sys, errno, json, os, psutil, string
import time
...
class ClientThread(threading.Thread): #separate thread for every user
	...
	def run(self):
		global server_info, hashrates, kicklist, connTimeout
		time = time.time()
		#New user connected
		username = ""
		...
		while time.time() - time < connTimeout
...
if not Path("config/config.ini").is_file():
	...
	conn_timeout = input("Enter client connection timeout (in seconds): ")
...
#Loading server config from INI if exists
	...
	conn_timeout = config['server']['conn_timeout']
