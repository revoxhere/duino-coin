...
Duino-Coin は多くの場合 MIT ライセンスの下で配布されています。詳細については、`LICENSE` ファイルを参照してください。サードパーティのインクルードファイルの中には、異なるライセンスを持つものがあります。それらの`LICENSE` 文（通常、ソースコード・ファイルの一番上にあります）を確認してください。
