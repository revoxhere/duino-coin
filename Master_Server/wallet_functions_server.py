...
PORTS = [
    # Pools (ideally should be run inside WALLET_PORT but globals dont update)
    #2810,
]
MOTD = """\
        ...
        if command[0] == "help":
            ...
            try:
                ...
                if confirm == "Y" or confirm == "y" or confirm == "":
                    ...
                    random = fastrandint(10000)
                    global_last_block_hash_cp = sha1(
                        bytes(str(command)+str(random),
                              encoding='ascii')).hexdigest()

                    admin_print("User balance is now " + str(balance), "TXID:" + global_last_block_hash_cp)
        ...
        try:
            ...
            try:
                ...
                if justswap_api.status_code == 200:
                    ...
                    duco_price_justswap = 0.0002867  # round(
                            ...
                            for user in rewards:
                                if rewards[user]:
                                    try:
                                        ...
                                        balances_to_update[user] = rewards[user]
                        admin_print("Pool synced", len(rewards), "rewards", len(poolWorkers), "workers")
                except Exception as e:
...
if __name__ == "__main__":
    ...
    admin_print("Duino-Coin Wallet Server is starting")
