...
def _derive_key(
    password: bytes, salt: bytes, iterations: int = ENCRYPTION_ITERATIONS
) -> bytes:
    kdf = PBKDF2HMAC(
...
def password_encrypt(
    message: bytes, password: str, iterations: int = ENCRYPTION_ITERATIONS
) -> bytes:
    salt = secrets.token_bytes(16)
    key = _derive_key(password.encode(), salt, ENCRYPTION_ITERATIONS)
    return b64e(
        b"%b%b%b"
        % (
            salt,
...
def password_decrypt(token: bytes, password: str) -> bytes:
    decoded = b64d(token)
    salt, ENCRYPTION_ITERATIONS, token = (
        decoded[:16],
        decoded[16:20],
        b64e(decoded[20:]),
    )
...
class LoginFrame(Frame):
    def __init__(self, master):
        ...
        self.ducoLabel = Label(
            self, background=FOREGROUND_COLOR, foreground=FONT_COLOR, image=self.duco
        )
        ...
        self.logbtn.grid(columnspan=2, sticky="nswe", padx=(5), pady=(5, 1))
        ...
        self.master.bind("<Return>", self._login_btn_clicked_bind)
        self.pack()
    ...
    def _login_btn_clicked(self):
        ...
        if username and password:
            ...
            soc.recv(3)
            soc.send(
                bytes("LOGI," + str(username) + "," + str(password), encoding="utf8")
            )
            ...
            if response[0] == "OK":
                ...
                messagebox.showerror(
                    title=get_string("login_error"), message=response[1]
                )
        else:
            messagebox.showerror(
        ...
        if emailS and usernameS and passwordS and confpasswordS:
            if passwordS == confpasswordS:
                ...
                if response[0] == "OK":
                    ...
                    messagebox.showerror(
                        title=get_string("register_error"), message=response[1]
                    )
            else:
                messagebox.showerror(
    ...
    def _register_btn_clicked(self):
        ...
        TEXT_FONT_BOLD = Font(register, size=12, weight="bold")
        TEXT_FONT = Font(register, size=12, weight="normal")
        ...
        tos_warning = "\n".join(
            l
            for line in tos_warning.splitlines()
        ...
        duco.image = duco
        ducoLabel = Label(
            register, background=FOREGROUND_COLOR, foreground=FONT_COLOR, image=duco
        )
...
def loading_window():
    ...
    try:
        ...
        pass
    TEXT_FONT = Font(loading, size=10, weight="bold")
    TEXT_FONT_BOLD = Font(loading, size=14, weight="bold")
    ...
    github.image = github
    githubLabel = Label(
        loading, image=github, background=BACKGROUND_COLOR, foreground=FONT_COLOR
    )
...
def transactions_window(handler):
    ...
    TEXT_FONT_BOLD_LARGE = Font(transactionsWindow, size=14, weight="bold")
    TEXT_FONT = Font(transactionsWindow, size=12, weight="normal")
    ...
    Label(
        ...
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ).grid(row=1, column=0, columnspan=2, sticky=S + W, pady=(5, 0), padx=5)
    ...
    scrollbar = Scrollbar(transactionsWindow, background=BACKGROUND_COLOR)
...
def currency_converter_window(handler):
    ...
    TEXT_FONT_BOLD = Font(calculatorWindow, size=12, weight="bold")
    TEXT_FONT_BOLD_LARGE = Font(calculatorWindow, size=14, weight="bold")
    TEXT_FONT = Font(calculatorWindow, size=12, weight="normal")
...
def statistics_window(handler):
    ...
    TEXT_FONT_BOLD_LARGE = Font(statsWindow, size=14, weight="bold")
    TEXT_FONT = Font(statsWindow, size=12, weight="normal")
    ...
    Label(
        statsWindow,
        text=get_string("difficulty") + ": " + str(statsApi["Current difficulty"]),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ...
    Label(
        statsWindow,
        text=get_string("mined_blocks") + ": " + str(statsApi["Mined blocks"]),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ...
    Label(
        statsWindow,
        text=get_string("network_hashrate") + ": " + str(statsApi["Pool hashrate"]),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ...
    Label(
        statsWindow,
        text=get_string("active_miners") + ": " + str(len(statsApi["Miners"])),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ...
    Label(
        statsWindow,
        text=get_string("registered_users") + ": " + str(statsApi["Registered users"]),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
...
def wrapper_window(handler):
    def Wrap():
        ...
        try:
            ...
            _ = soc.recv(10)
            soc.send(bytes("WRAP," + str(amount) + "," + str(pub_key), encoding="utf8"))
            soc.close()
            sleep(2)
            wrapperWindow.quit()
    ...
    try:
        ...
        messagebox.showerror(
            title=get_string("wrapper_error_title"), message=get_string("wrapper_error")
        )
    else:
        if TRONPY_ENABLED:
            ...
            askWrapAmount = Label(
                wrapperWindow, text=get_string("wrapper_amount_to_wrap") + ":"
            )
...
def unwrapper_window(handler):
    ...
    try:
        ...
        if TRONPY_ENABLED:
            ...
            askAmount = Label(unWrapperWindow, text=get_string("unwrap_amount"))
            ...
            askPassphrase = Label(unWrapperWindow, text=get_string("ask_passphrase"))
            ...
            wrapButton = Button(
                unWrapperWindow, text=get_string("unwrap_duco"), command=UnWrap
            )
...
def settings_window(handler):
    def _wrapperconf():
        if TRONPY_ENABLED:
            ...
            def setwrapper():
                if privkey_input and passphrase_input:
                    ...
                    try:
                        ...
                        privkeyfile = open(str(resources + "DUCOPrivKey.encrypt"), "w")
                        privkeyfile.write(
                            str(
                                password_encrypt(priv_key.encode(), passphrase).decode()
                            )
                        )
                        privkeyfile.close()

                        pubkeyfile = open(str(resources + "DUCOPubKey.pub"), "w")
                        pubkeyfile.write(pub_key)
                        pubkeyfile.close()

                        Label(wrapconfWindow, text=get_string("wrapper_success")).pack()
                        wrapconfWindow.quit()
            ...
            askprivkey = Label(wrapconfWindow, text=get_string("ask_private_key"))
            askprivkey.grid(row=1, column=0, sticky=N + W)

            privkey_entry = Entry(
                wrapconfWindow, font=TEXT_FONT, textvariable=privkey_input
            )
            privkey_entry.grid(row=2, column=0, sticky=N + W)

            askpassphrase = Label(wrapconfWindow, text=get_string("passphrase"))
            askpassphrase.grid(row=3, column=0, sticky=N + W)

            passphrase_entry = Entry(
                wrapconfWindow, font=TEXT_FONT, textvariable=passphrase_input
            )
            ...
            if oldpasswordS != newpasswordS:
                if oldpasswordS and newpasswordS and confpasswordS:
                    if newpasswordS == confpasswordS:
                        ...
                        soc.send(
                            bytes(
                                "LOGI," + str(username) + "," + str(password),
                                encoding="utf8",
                            )
                        )
                        soc.recv(2)
                        soc.send(
                            bytes(
                                "CHGP," + str(oldpasswordS) + "," + str(newpasswordS),
                                encoding="utf8",
                            )
                        )
                        response = soc.recv(128).decode("utf8").rstrip("\n").split(",")
    ...
    TEXT_FONT = Font(settingsWindow, size=12, weight="normal")
    TEXT_FONT_BOLD_LARGE = Font(settingsWindow, size=12, weight="bold")
    ...
    Label(
        settingsWindow,
        text=get_string("logged_in_as") + ": " + str(username),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ...
    Label(
        settingsWindow,
        text=get_string("wallet_version") + ": " + str(VERSION),
        font=TEXT_FONT,
        background=BACKGROUND_COLOR,
        foreground=FONT_COLOR,
    ...
    github.image = github
    githubLabel = Label(
        settingsWindow, image=github, background=BACKGROUND_COLOR, foreground=FONT_COLOR
    )
...
def get_balance():
    ...
    try:
        ...
        try:
            gtxl = {}
            soc.send(bytes("GTXL," + str(username) + ",7", encoding="utf8"))
        ...
        if oldbalance != balance:
            ...
            if float(balance) != float(difference):
                if dif_with_unpaid >= MIN_TRANSACTION_VALUE or dif_with_unpaid < 0:
                    now = datetime.now()
    ...
    try:
        ...
        if profit_array[2] != 0:
            sessionprofittext.set(
                get_string("session") + ": " + str(profit_array[0]) + " ᕲ"
            )
            minuteprofittext.set(
                "≈" + str(profit_array[1]) + " ᕲ/" + get_string("minute")
            )
            hourlyprofittext.set(
                "≈" + str(profit_array[2]) + " ᕲ/" + get_string("hour")
            )
            dailyprofittext.set(
                "≈"
                + str(profit_array[3])
...
def send_funds_protocol(handler):
    ...
    if MsgBox == "yes":
        ...
        soc.send(
            bytes(
                "SEND," + "-" + "," + str(recipientStr) + "," + str(amountStr),
                encoding="utf8",
            )
        )
...
class Wallet:
    def __init__(self, master):
        ...
        TEXT_FONT_BOLD_LARGE = Font(size=12, weight="bold")
        TEXT_FONT_BOLD = Font(size=18, weight="bold")
        TEXT_FONT = Font(size=12, weight="normal")
        ...
        Label(
            master,
            text=get_string("uppercase_duino_coin_wallet") + ": " + str(username),
            font=TEXT_FONT_BOLD_LARGE,
            foreground=FOREGROUND_COLOR,
            background=BACKGROUND_COLOR,
        ...
        sendLabel.grid(
            row=8, column=0, sticky=N + S + E + W, columnspan=4, padx=(5), pady=(1, 2)
        )
        ...
        calculatorLabel = Label(
            master, image=calculator, background=BACKGROUND_COLOR, foreground=FONT_COLOR
        )
        ...
        stats.image = stats
        statsLabel = Label(
            master, image=stats, background=BACKGROUND_COLOR, foreground=FONT_COLOR
        )
        statsLabel.grid(
        ...
        settingsLabel = Label(
            master, image=settings, background=BACKGROUND_COLOR, foreground=FONT_COLOR
        )
        settingsLabel.grid(
