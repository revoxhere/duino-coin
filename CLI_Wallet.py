        ...
        if res.status_code == 200:  # Check for response
            ...
            try:  # Try to connect
                ...
                if jsonapi.status_code == 200:  # Check for reponse
                    content = (
                        jsonapi.content.decode()
                    )  # Read content and split into lines
                    contentjson = json.loads(content)
                    ducofiat = float(contentjson["Duco price"])
                else:
                    ducofiat = (
                        0.0025  # If json api request fails, wallet will use this value
                    )
    ...
    if not Path("CLIWallet_config.cfg").is_file():
        ...
        if config["wrapper"]["use_wrapper"] == "true" and tronpy_installed:
            ...
            if config["wrapper"]["use_custom_passphrase"] == "true":
                ...
                try:
                    priv_key = str(
                        password_decrypt(config["wrapper"]["priv_key"], passphrase)
                    )[2:66]
                ...
                priv_key = str(
                    password_decrypt(
                        config["wrapper"]["priv_key"], config["wallet"]["password"]
                    )
                )[2:66]
        ...
        while True:
            ...
            s.send(
                bytes(
                    "LOGI," + str(username) + "," + str(password) + str(",placeholder"),
                    encoding="utf8",
                )
            )
        ...
        while True:
            while True:
                ...
                try:
                    ...
                    pass
            print(
                Style.RESET_ALL + Style.BRIGHT + Fore.YELLOW + "\nDuino-Coin CLI Wallet"
            )
            ...
            if use_wrapper:
                ...
                pendingbalance = float(
                    wduco.functions.pendingWithdrawals(pub_key, username)
                ) / (10 ** 6)
            ...
            print(
                Style.RESET_ALL + Fore.YELLOW + "Type `help` to list available commands"
            )

            command = input(
                Style.RESET_ALL + Fore.WHITE + "DUCO Console ᕲ " + Style.BRIGHT
            )
            ...
            if command == "refresh":
                ...
                s.send(
                    bytes(
                        "SEND,-," + str(recipient) + "," + str(amount), encoding="utf8"
                    )
                )
                while True:
                ...
                s.send(
                    bytes(
                        "CHGP," + str(oldpassword) + "," + str(newpassword),
                        encoding="utf8",
                    )
                )
                ...
                if use_wrapper:
                    ...
                    if float(amount) >= 10 and float(amount) <= balance:
                        ...
                        s.send(
                            bytes(
                                "WRAP," + str(amount) + "," + str(tron_address),
                                encoding="utf8",
                            )
                        )
                ...
                if use_wrapper:
                    txn = (
                        wduco.functions.cancelWithdrawals(pub_key, username)
                        .with_owner(pub_key)
                        .fee_limit(5_000_000)
                        .build()
                        .sign(PrivateKey(bytes.fromhex(priv_key)))
                ...
                if use_wrapper:
                    ...
                    s.send(
                        bytes(
                            "UNWRAP," + str(pendingvalues) + "," + str(pub_key),
                            encoding="utf8",
                        )
                    )
                ...
                if use_wrapper:
                    ...
                    if float(amount) <= wbalance:
                        txn = (
                            wduco.functions.transfer(
                                recipient, int(float(amount) * 10 ** 6)
                            )
                            .with_owner(pub_key)
                            .fee_limit(5_000_000)
                            .build()
                ...
                if float(SERVER_VER) > VER:
                    ...
                    print(Style.RESET_ALL + Fore.WHITE + "Client is up-to-date")
