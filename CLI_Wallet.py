        ...
        if res.status_code == 200:  # Check for response
            ...
            try:  # Try to connect
                ...
                jsonapi = requests.get(
                    "https://raw.githubusercontent.com/revoxhere/duco-statistics/master/api.json", data=None)
                ...
                print(Style.RESET_ALL + Fore.RED +
                      "Cannot connect to the server. It is probably under maintenance or temporarily down.\nRetrying in 15 seconds.")
                time.sleep(15)
    ...
    if not Path("CLIWallet_config.cfg").is_file():  # Initial configuration section
        ...
        if int(choice) <= 1:
            ...
            while server_timeout:
                try:
                    s.send(bytes("LOGI," + str(username) + "," +
                                 str(password) + str(",placeholder"), encoding="utf8"))
                    ...
                    if loginFeedback[0] == "OK":
                        ...
                        print(Style.RESET_ALL + Fore.RED + "Couldn't login, reason: " +
                              Style.BRIGHT + str(loginFeedback[1]))
                        time.sleep(15)
                        os._exit(1)
        ...
        if int(choice) == 2:
            ...
            pconfirm = getpass.getpass(
                prompt=Style.RESET_ALL + Fore.YELLOW + "Confirm your password: " + Style.BRIGHT, stream=None)
            email = input(Style.RESET_ALL + Fore.YELLOW +
                          "Enter your e-mail address (used for exchange and support): " + Style.BRIGHT)
            if password == pconfirm:
                while True:
                    s.send(bytes("REGI," + str(username) + "," +
                                 str(password) + "," + str(email), encoding="utf8"))
        ...
        if config["wrapper"]["use_wrapper"] == "true" and tronpy_installed:
            ...
            if config["wrapper"]["use_custom_passphrase"] == "true":
                passphrase = getpass.getpass(prompt=Style.RESET_ALL + Fore.YELLOW +
                                             "Passphrase for decrypting private key : " + Style.BRIGHT, stream=None)
                try:
                    priv_key = str(password_decrypt(
                        config["wrapper"]["priv_key"], passphrase))[2:66]
                except InvalidToken:
                ...
                priv_key = str(password_decrypt(
                    config["wrapper"]["priv_key"], config["wallet"]["password"]))[2:66]
        ...
        while True:
            ...
            print(Style.RESET_ALL + Fore.YELLOW + "You have " +
                  Style.BRIGHT + str(balance) + " DUCO")
            print(Style.RESET_ALL + Fore.YELLOW + "Which is about " +
                  Style.BRIGHT + str(balanceusd) + " USD")
            print(Style.RESET_ALL + Fore.YELLOW + "DUCO price : " +
                  Style.BRIGHT + str(ducofiat) + " USD")
            if use_wrapper:
                print(Style.RESET_ALL + Fore.YELLOW + "You have " +
                      Style.BRIGHT + str(wbalance) + " wDUCO")
                pendingbalance = float(
                    wduco.functions.pendingWithdrawals(pub_key, username))/(10**6)
                if pendingbalance > 0:
                    print(Style.RESET_ALL + Fore.YELLOW + "Pending unwraps " +
                          Style.BRIGHT + str(pendingbalance) + " wDUCO")
                print(Style.RESET_ALL + Fore.YELLOW +
                      "Tron address for receiving : " + Style.BRIGHT + str(pub_key))
                print(Style.RESET_ALL + Fore.YELLOW +
                      "TRX balance (useful for fees) : " + Style.BRIGHT + str(trx_balance))
            print(Style.RESET_ALL + Fore.YELLOW +
                  "Type `help` to list available commands")
            command = input(Style.RESET_ALL + Fore.WHITE +
                            "DUCO Console $ " + Style.BRIGHT)
            if command == "refresh":
                continue

            elif command == "send":
                recipient = input(Style.RESET_ALL + Fore.WHITE +
                                  "Enter recipients' username: " + Style.BRIGHT)
                try:
                    amount = float(input(
                        Style.RESET_ALL + Fore.WHITE + "Enter amount to transfer: " + Style.BRIGHT))
                except ValueError:
                    print("NO, Amount should be numeric... aborting")
                    continue

                s.send(bytes("SEND,deprecated,"+str(recipient) +
                             ","+str(amount), encoding="utf8"))
                while True:
                    message = s.recv(1024).decode()
                    print(Style.RESET_ALL + Fore.BLUE +
                          "Server message: " + Style.BRIGHT + str(message))
                    break

            elif command == "changepass":
                oldpassword = input(
                    Style.RESET_ALL + Fore.WHITE + "Enter your current password: " + Style.BRIGHT)
                newpassword = input(
                    Style.RESET_ALL + Fore.WHITE + "Enter new password: " + Style.BRIGHT)
                s.send(bytes("CHGP," + str(oldpassword) +
                             "," + str(newpassword), encoding="utf8"))
                while True:
                    message = s.recv(1024).decode()
                    print(Style.RESET_ALL + Fore.BLUE +
                          "Server message: " + Style.BRIGHT + str(message))
                ...
                if not config["wrapper"]["use_wrapper"] == "true" and tronpy_installed:
                    ...
                    try:
                        ...
                        if choice <= 1:
                            ...
                            print(
                                "How do you want to encrypt private keey within config file ?")
                            ...
                            while incorrect_value:
                                ...
                                if encryption_choice <= 1:
                                    config['wrapper'] = {"use_wrapper": "true", "priv_key": str(password_encrypt(
                                        priv_key.encode(), password).decode()), "pub_key": pub_key, "use_custom_passphrase": "false"}
                                    # Write data to file
                                    ...
                                    print(
                                        "Input your passphrase for encrypting private key !")
                                    passphrase = input("passphrase : ")
                                    config['wrapper'] = {"use_wrapper": "true", "priv_key": str(password_encrypt(
                                        priv_key.encode(), passphrase).decode()), "pub_key": pub_key, "use_custom_passphrase": "true"}
                                    # Write data to file
                                    with open("CLIWallet_config.cfg", "w") as configfile:
                                        config.write(configfile)
                                        print("Success !")
                            ...
                            try:
                                ...
                                while incorrect_value:
                                    try:
                                        encryption_choice = int(input(
                                            "1 - encrypt it using DUCO password (low security)\n2 - use your custom passphrase (more secure)\n"))
                                        incorrect_value = False
                                    except ValueError:
                                        print("Error, should be numeric")
                                        incorrect_value = True
                        ...
                        print(Style.RESET_ALL + Fore.WHITE +
                              "Incorrect value was provided" + Style.bright)
                elif not tronpy_installed:
                    print(now.strftime(
                        "%H:%M:%S ") + "Tronpy is not installed. Please install it using: python3 -m pip install tronpy")

            elif command == "wrap":
                if use_wrapper:
                    try:
                        amount = float(input(
                            Style.RESET_ALL + Fore.WHITE + "Enter amount to wrap (minimum is 10): " + Style.BRIGHT))
                    except ValueError:
                        print("NO, Amount should be numeric... aborting")
                        continue
                    ...
                    if float(amount) >= 10 and float(amount) <= balance:
                        tron_address = input(
                            Style.RESET_ALL + Fore.WHITE + "Enter tron address, or leave blank to choose local wallet: " + Style.BRIGHT)
                        if tron_address == "":
                            tron_address = pub_key
                        s.send(bytes(str("WRAP,")+str(amount)+str(",") +
                                     str(tron_address), encoding='utf8'))
                ...
                if use_wrapper:
                    ...
                    try:
                        amount = float(
                            input(Style.RESET_ALL + Fore.WHITE + "Enter amount to unwrap: " + Style.BRIGHT))
                    except ValueError:
                    ...
                    if txn_success or amount <= pendingvalues:
                        s.send(bytes(str("UNWRAP,")+str(amount) +
                                     str(",")+str(pub_key), encoding='utf8'))
                    else:
                ...
                if use_wrapper:
                    pendingvalues = float(
                        wduco.functions.pendingWithdrawals(pub_key, username))/(10**6)
                    s.send(bytes(str("UNWRAP,")+str(pendingvalues) +
                                 str(",")+str(pub_key), encoding='utf8'))
                ...
                if use_wrapper:
                    recipient = input(
                        Style.RESET_ALL + Fore.WHITE + "Enter recipients' TRON address: " + Style.BRIGHT)
                    try:
                        amount = float(input(
                            Style.RESET_ALL + Fore.WHITE + "Enter amount to transfer: " + Style.BRIGHT))
                    except ValueError:
                    ...
                    if float(amount) <= wbalance:
                        ...
                        txn = txn.broadcast()
                        print(
                            "Transaction submitted to TRON network\nTXid :", txn.txid)
                ...
                print(Style.RESET_ALL + Fore.WHITE +
                      "Duino-Coin CLI Wallet is made with <3 by Duino-Coin community")
                print(Style.RESET_ALL + Fore.WHITE +
                      "This is version "+str(VER))
                print(Style.RESET_ALL + Fore.WHITE +
                      "And is distributed under MIT license")
                print(Style.RESET_ALL + Fore.WHITE +
                      Style.BRIGHT + "https://duinocoin.com")
                print(Style.RESET_ALL + Fore.WHITE + "Stay safe everyone!")
                print(Style.RESET_ALL + Fore.WHITE + "Server version : " +
                      Style.BRIGHT + SERVER_VER + Style.RESET_ALL)
                if float(SERVER_VER) > VER:
                    print(Style.RESET_ALL + Fore.YELLOW + "Server is " + Fore.WHITE + Style.BRIGHT + SERVER_VER + Fore.YELLOW + Style.RESET_ALL +
                          ", but client is " + Style.BRIGHT + Fore.WHITE + VER + Style.RESET_ALL + Fore.YELLOW + ", you should consider downloading last release")
                else:
                    print(Style.RESET_ALL + Fore.WHITE +
                          "Client is up-to-date :D")
                ...
                print(Style.RESET_ALL + Fore.BLUE + Style.BRIGHT +
                      "Feel free of donating for helping to maintain DUCO and wDUCO (wrapping/unwrapping involves fees for maintainers) ?")
                print(Style.RESET_ALL + Fore.YELLOW + "TRON and tokens : " + Style.RESET_ALL + Fore.WHITE + Style.BRIGHT +
                      "TY5wfM6JsYKEEMfQR3RBQBPKfetTpf7nyM" + Style.RESET_ALL + Fore.YELLOW + " (wrapper's address)")
                print("Duino-Coin : " + Style.RESET_ALL + Fore.WHITE + Style.BRIGHT +
                      "revox" + Style.RESET_ALL + Fore.YELLOW + " (revox, DUCO lead developer)")
                print("Duino-Coin : " + Style.RESET_ALL + Fore.WHITE + Style.BRIGHT +
                      "acridkiller" + Style.RESET_ALL + Fore.YELLOW + " (Acrid, duco faucet maintainer)")
                print("Duino-Coin : " + Style.RESET_ALL + Fore.WHITE + Style.BRIGHT +
                      "Yanis" + Style.RESET_ALL + Fore.YELLOW + " (wDUCO developer)")
